...
class GAN():
    ...
    def build_generator(self):
        ...
        model_enc.add(Conv2D(64,(4,4),(2,2),padding='same'))
        model_enc.add(BatchNormalization())
        model_enc.add(LeakyReLU(alpha=0.2))
        model_enc.add(Conv2D(128,(4,4),(2,2),padding='same'))
        model_enc.add(BatchNormalization())
        model_enc.add(LeakyReLU(alpha=0.2))
        model_enc.add(Conv2D(256,(4,4),(2,2),padding='same'))
        model_enc.add(BatchNormalization())
        model_enc.add(LeakyReLU(alpha=0.2))
        model_enc.add(Conv2D(512,(4,4),(2,2),padding='same'))
        model_enc.add(BatchNormalization())
        model_enc.add(LeakyReLU(alpha=0.2))
        model.add(Flatten())
        ...
        model_dec.add(Conv2DTranspose(256,(4,4),(2,2),padding='same'))
        model_dec.add(BatchNormalization())
        model_dec.add(LeakyReLU(alpha=0.2))
        model_dec.add(Conv2DTranspose(128,(4,4),(2,2),padding='same'))
        model_dec.add(BatchNormalization())
        model_dec.add(LeakyReLU(alpha=0.2))
        model_dec.add(Conv2DTranspose(64,(4,4),(2,2),padding='same'))
        model_dec.add(BatchNormalization())
        model_dec.add(LeakyReLU(alpha=0.2))
        model_dec.add(Conv2DTranspose(1,(4,4),(2,2),padding='same'))
        model_dec.add(BatchNormalization())
        model_dec.add(Activation('sigmoid'))

        noise = Input(shape=self.img_shape)
        label = Input(shape=self.img_shape)

        model_input = merge.multiply([noise, label])
        enc = model_enc(model_input)
        dec = model(enc)
        img = model_dec(merge.add([dec,enc]))
